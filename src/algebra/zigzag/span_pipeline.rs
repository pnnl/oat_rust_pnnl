...
/// ```
pub fn interval_decomposition_for_zigzag_of_spans< RingOperator >( 
        spans:                     Vec< Span< RingOperator::Element > >, 
        ...
        RingOperator::Element:        Clone + std::fmt::Debug + Eq + Ord,
{
    ...
    for single_bar_ledger in diagonalization.list_of_single_bar_basis_vector_index_ledgers() {
        ...
        for (vertex, basis_vector_index) in single_bar_ledger.iter() {
            let row_indexed_by_usize       =   diagonalization.bases()[ vertex ].row_ref( basis_vector_index );

            let homology_basis_matrix     =     MatrixAlgebraPacket::with_default_order(
                                                    & homology_cycle_bases[ vertex ], 
                                                    ring_operator.clone()
                                                );
            let chain   =   homology_basis_matrix
                                                            .multiply_with_row_vector(row_indexed_by_usize)
                                                            .collect();
            chains.push(chain);
        }
